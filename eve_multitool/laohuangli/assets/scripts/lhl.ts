...
function updateAll(): void {
  const d = new Date();
  const now: SolarTime = SolarTime.fromYmdHms(
    d.getFullYear(),
    d.getMonth() + 1,
    d.getDate(),
    d.getHours(),
  ...
  const hour: SixtyCycleHour = now.getSixtyCycleHour();
  const day: SixtyCycleDay = now.getSolarDay().getSixtyCycleDay();
  ...
  const elementsToUpdate: ElementUpdate[] = [
    ...
    { id: "ttime", content: hour.getName() },
    {
      id: "yearmonth",
      content:
        day.getSolarDay().getSolarMonth().getSolarYear().getName() +
        day.getSolarDay().getSolarMonth().getName(),
    },
    {
      id: "tyearmonth",
      content: day.getYear().getName() + "年" + day.getMonth().getName() + "月",
    },
  ...
  const lucky =
    day.getSolarDay().getLunarDay().getMinorRen().getLuck().getIndex() === 0;
  ...
  if (table) {
    table.style.borderColor = lucky ? "green" : "darkred";
  }
}

function updateColor(e: HTMLElement, good: boolean): void {
  if (good === true) {
