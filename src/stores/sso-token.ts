...
export const useTokenStore = defineStore('sso-token', () => {
  ...
  const spec = computed(async () => {
    ...
    if (!localSpec) {
      const wellKnownEndpoint = import.meta.env.VITE_EVE_ESI_WELL_KNOWN_ENDPOINT || "https://login.eveonline.com/.well-known/oauth-authorization-server"
  ...
  const authEndpointUrl = computed(async () => (await spec.value).authorization_endpoint || "https://login.eveonline.com/v2/oauth/authorize")
  const tokenEndpointUrl = computed(async () => (await spec.value).token_endpoint || "https://login.eveonline.com/v2/oauth/token")
  const jwksEndpointUrl = computed(async () => (await spec.value).jwks_uri || "https://login.eveonline.com/oauth/jwks")
  ...
  async function callbackHandler(authorizationCode: string, state: string): Promise<string | { name: string }> {
  ...
  async function exchangeToken(authorizationCode: string): Promise<string | undefined> {
    ...
    await axios.post(await tokenEndpointUrl.value, new URLSearchParams({
  ...
  return { accessToken, refreshToken, generateAuthUrl, callbackHandler, validateToken, clearTokens }
})
